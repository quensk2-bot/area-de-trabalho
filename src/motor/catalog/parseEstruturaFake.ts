...
export function parseEstruturaFake(filePath: string): CatalogoLoadResult<CatalogoEstruturaFake> {
  ...
  const sheetName = ["Planilha1", "DADOS", "Estrutura Fake"].find((n) => workbook.SheetNames.includes(n)) ?? workbook.SheetNames[0];
  const rows = XLSX.utils.sheet_to_json<SheetRow>(workbook.Sheets[sheetName], { defval: null });

  const itens: CatalogoEstruturaFake[] = rows
    .map((r) => ({
    ...
    .filter((i) => i.loja && !i.loja.includes("FILTRAR") && i.seqproduto);
