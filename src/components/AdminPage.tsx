...
export default function AdminPage() {
  ...
  async function salvarUsuario(e: React.FormEvent) {
    ...
    const usuarioAtualizado: UsuarioAuto = {
    ...
    if (
      false &&
      usuarioAtualizado.grupo_id &&
      (usuarioAtualizado.nivel === "N2" || usuarioAtualizado.nivel === "N3")
    ) {
      const { data: modelos, error: modelosError } = await supabase
        ...
        .eq("grupo_id", usuarioAtualizado.grupo_id)
        .eq("departamento_id", usuarioAtualizado.departamento_id)
        .eq("setor_id", usuarioAtualizado.setor_id);

      if (modelosError) {
        console.error("Erro ao carregar modelos para o grupo na edição:", modelosError.message);
        ...
        const modelosFiltrados = (modelos as RotinaPadraoAuto[]).filter((m) => {
          ...
          if (usuarioAtualizado.regional_id == null) return false;
          const regionais = m.regionais_ids
            .map((id) => Number(id))
            .filter((id) => Number.isFinite(id));
          return regionais.includes(usuarioAtualizado.regional_id);
        });

        let modelosPendentes = modelosFiltrados;
        const modelosIds = modelosFiltrados.map((m) => String(m.id));

        if (modelosIds.length > 0) {
          const { data: existentes, error: existentesError } = await supabase
            .from("rotinas")
            .select("rotina_padrao_id")
            .eq("responsavel_id", usuarioSelecionado.id)
            .in("rotina_padrao_id", modelosIds as any);

          if (existentesError) {
            console.error("Erro ao carregar rotinas existentes do usuário:", existentesError.message);
          } else {
            const existentesIds = new Set(
              (existentes ?? [])
                .map((r: any) => (r.rotina_padrao_id == null ? null : String(r.rotina_padrao_id)))
                .filter(Boolean)
            );
            modelosPendentes = modelosFiltrados.filter((m) => !existentesIds.has(String(m.id)));
          }
        }

        if (modelosPendentes.length > 0) {
          const grupoInfo = grupos.find((g) => g.id === usuarioAtualizado.grupo_id) ?? null;
          ...
          const resultado = await criarRotinasAutomaticasParaUsuario({
            modelos: modelosPendentes,
            usuario: usuarioAtualizado,
            criadorId,
            grupoRegionalId,
          });
          if (resultado.erros.length > 0) {
            console.error("Erros ao gerar rotinas na edição:", resultado.erros);
            const preview = resultado.erros.slice(0, 3).join(" | ");
            alert("Usuário atualizado, mas houve erro ao gerar rotinas automáticas. " + preview);
          }
        }
      }
    }
