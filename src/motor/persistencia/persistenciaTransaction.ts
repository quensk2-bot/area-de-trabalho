...
export type PersistirLoteOptions = {
  ...
  ativar?: boolean;
  /** Forcar versao explicita; caso omitido, calcula proxima versao. */
  versao?: number;
};

/**
 * Persistencia transacional com compensacao (rollback DELETE) em falha.
 *
 * Limitacao PostgREST: nao ha transacao SQL longa no cliente JS.
 * Em erro apos criar execucao_motor, executa DELETE CASCADE via rollbackExecucaoPorId.
 * Ativacao usa RPC motor_v7.ativar_execucao (transacional no PostgreSQL).
 */
export async function persistirLoteMotor(
  ...
  // Idempotencia A: mesmo hash + concluida
...
export { validarEntradaPersistencia };
